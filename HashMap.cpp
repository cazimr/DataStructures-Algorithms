...
//HASH MAPA            HASH MAPA              HASH MAPA               HASH MAPA

template <typename TKljuc, typename TVr>
class HashMapa: public Mapa<TKljuc,TVr>{
    unsigned int (*hesiraj)(TKljuc,unsigned int);
    TVr* elementi;
    TKljuc** kljucevi;
    unsigned int kapacitet, brEl;
    TKljuc* obrisano;
    ...
    void uduplajKapacitet(){
        TVr* elem2 = new TVr[kapacitet*10];
        TKljuc** kljuc2 = new TKljuc*[kapacitet*10];
        
        for(int i=0; i<kapacitet*10; i++) kljuc2[i]=nullptr;
        
        for(int i=0; i<kapacitet; i++){
            if(kljucevi[i]!=nullptr && kljucevi[i]!=obrisano) {
                int indeks = hesiraj(*(kljucevi[i]),kapacitet*10);
                while(kljuc2[indeks]!=nullptr){
                    indeks=(indeks+1)%(kapacitet*10);
                }
                kljuc2[indeks]=kljucevi[i]; //sada sa novim kapacitetom ce imati novo mjesto u tabeli
                elem2[indeks] = elementi[i];
            }
        }
        kapacitet*=10;
        delete[] elementi;
...
public:
    ...
    HashMapa(): kapacitet(10), brEl(0), hesiraj(nullptr){
        elementi = new TVr[kapacitet]();
        kljucevi = new TKljuc*[kapacitet];
        obrisano= new TKljuc();
        inicijaliziraj();
    }
    
    HashMapa(const HashMapa& kopija):kapacitet(kopija.kapacitet), brEl(kopija.brEl), hesiraj(kopija.hesiraj){
        elementi = new TVr[kapacitet]();
        kljucevi = new TKljuc*[kapacitet];
        obrisano = new TKljuc();
        inicijaliziraj();
        for(int i=0; i<kapacitet; i++){
            elementi[i]=kopija.elementi[i];
            if(kopija.kljucevi[i]!=nullptr)
                kljucevi[i]=new TKljuc(*(kopija.kljucevi[i]));
        }
    }
    ...
    HashMapa& operator =(HashMapa mapa){
        ...
        for(int i=0; i<kapacitet; i++){
            if(kljucevi[i]!=obrisano) delete kljucevi[i];
        } 
        delete[] elementi;
        delete[] kljucevi;
        delete obrisano;
        ...
        brEl=mapa.brEl;
        mapa.brEl=0;
        obrisano = mapa.obrisano;
        mapa.obrisano=nullptr;
        elementi=mapa.elementi;
        mapa.elementi=nullptr;
        kljucevi=mapa.kljucevi;
        mapa.kljucevi=nullptr;
    ...
    void obrisi(){
        ...
        delete[] elementi;
        delete[] kljucevi;
        //vracamo u pocetni stadij kao da se tek kreirala hashMapa
        kapacitet=10;
        brEl=0;
        elementi = new TVr[kapacitet];
        kljucevi = new TKljuc*[kapacitet];
    ...
    void definisiHashFunkciju(unsigned int (*param)(TKljuc,unsigned int)){
        hesiraj=param;
    }
    
    
    void obrisi(const TKljuc& kljuc){
        if(hesiraj==nullptr) throw logic_error("Nije definirana funkcija hesiranja!");
        int j=hesiraj(kljuc,kapacitet),i=j;
        do{
            if(kljucevi[j] == nullptr) throw logic_error("Element s datim kljucem ne postoji!"); //znaci da ga nema sigurno, pa nemamo sta ni obrisati
            else if(*(kljucevi[j])!=kljuc){ 
                j=(j+1)%kapacitet; //linearno ispitivanje, pomjeramo se dalje
            }
            else if(*(kljucevi[j])==kljuc){
                delete kljucevi[j];
                kljucevi[j]=obrisano;
        ...
        throw logic_error("Element s datim kljucem ne postoji!");
    }
    
    
     TVr& operator [](const TKljuc& kljuc){
         if(hesiraj==nullptr) throw logic_error("Funkcija hesiranja nije definisana!");
         int j=hesiraj(kljuc,kapacitet),i=j,indi=-1;
         do{
            

            if (kljucevi[j]==obrisano && indi==-1) indi=j;
            
             
            else if(kljucevi[j] == nullptr) break;
        ...
        if(indi==-1) indi=j; //break je aktiviran i nadjen je nullptr bez da je prethodno bio neki slobodni na obrisanom mjestu 
        kljucevi[indi] = new TKljuc(kljuc); //u protivnom ce indi biti prvo obrisano mjesto u tabeli
        brEl++;
        if(brEl==kapacitet) uduplajKapacitet();
        elementi[indi]=TVr{};
        return elementi[indi];
     }
     
     
     TVr operator [](const TKljuc& kljuc) const{
         if(hesiraj==nullptr) throw logic_error("Funkcija hesiranja nije definirana!");
         int j=hesiraj(kljuc,kapacitet),i=j;
         do{
            if(kljucevi[j] == nullptr || kljucevi[j] == obrisano)
            {
                return TVr{};
            }
            else if(*(kljucevi[j])!=kljuc){ 
...
//BINSTABLO MAPA      BINSTABLO MAPA        BINSTABLO MAPA          BINSTABLO MAPA           BINSTABLO MAPA
...
unsigned int funkcijaHesiranjaa(int a, unsigned int max){
    return a%max;
}
...
int main(){
    ...
    hashMapa.definisiHashFunkciju(funkcijaHesiranjaa);
    srand(time(NULL));
    for(int i=0; i<1500; i++){
        nizMapa[i] = rand()%20;
        binMapa[i] = (rand()+1)%12;
        hashMapa[i] = (rand()+42)%9;
    }
    ...
    ukupno = ((vrijeme2-vrijeme1)*1.)/((CLOCKS_PER_SEC/1000)*1.);
    cout<<endl;
    cout<<"Vrijeme izvrsavanja pristupa postojecem elementu hashMape iznosi: "<<ukupno<<"ms"<<endl;
    ...
    //Za hash mapu imamo, uz uslov da je jako brza funkcija hesiranja, direktan indeks gdje trebamo smjestiti elemetn u niz
    //uz potencijalnu koliziju koja ce se opet rijesiti relativno brzo. Dakle indeks se dobija brzom funkcijom hesiranja i
    // element se smijesta odmah tu. Tako isto i pristup elementu je jako brz jer direktno dobijamo njegov indeks
    //putem funkcije hesiranja, opet uz rjesavanje kolizije otvorenim adresiranjem, za razliku od nizMape 
    // i binStabloMape gdje smo morali koristiti ili binarnu pretragu ako smo optimalno uradili ili
    // ici sekvencijalno i traziti prvo slobodno mjesto da umetnemo. Tako da je svakako ocekivano da ce iznimno
    //brza biti hashMapa od ostale 2
    ...
    ukupno = ((vrijeme2-vrijeme1)*1.)/((CLOCKS_PER_SEC/1000)*1.);
    cout<<endl;
    cout<<"Vrijeme izvrsavanja dodavanja novog elementa u hashMapu iznosi: "<<ukupno<<"ms"<<endl;
    return 0;
}